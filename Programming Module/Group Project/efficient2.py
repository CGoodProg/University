...
infile=open("Happy_Numbers.txt",'r')
hap_num=[]
for i in infile:
#    data=input(infile)
#    x=data.split(',')
...
#    
x=hap_num
y=np.arange(0,10**3,10)

#print x,y
plt.figure(1)
plt.plot(x,y)
plt.xlabel("n range")
plt.ylabel("h(n) cumulative frequency")
plt.title("Cumulative Frequency of Happy Numbers")
#plt.axis([0,rnge,0,])
#plt.savefigure("cumulative frequency of happy numbers")
plt.show()
